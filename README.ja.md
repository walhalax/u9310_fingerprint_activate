...
Fujitsu LIFEBOOK U9310 シリーズ (U9311 / U9312 を含む) の **NB-2033-U 指紋センサー** を Ubuntu 24.04+ で有効化するための、単一目的のセットアップスクリプトです。
...
> このリポジトリは指紋セットアップ **だけ** に絞っています。apt upgrade / PPA 追加 / Tailscale / crush などの周辺セットアップは別リポジトリで扱う想定です。
...
1. **指紋を登録**: 設定 → ユーザー → 自分のユーザー → 指紋認証 → 「+」→ 5回スキャン
2. **GDM ログイン画面でテスト**: ログアウト後、サインイン画面で「指を置いてください」が表示されるか確認
...
- `/etc/pam.d/gdm-password` の `@include common-auth` の前に `auth sufficient pam_fprintd.so` を挿入 → 指紋成功で即ログイン、失敗時はパスワードにフォールバック
